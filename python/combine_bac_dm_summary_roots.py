#!/usr/bin/env python3
"""
Combine already-produced BAC DM summary ROOT files.

This version intentionally uses the *original config_dm_summary*.yaml* that was
...
def find_plotcfg(directory):
    """
    Find the exact DM plot configuration saved with the summary.

    Preferred route:
      1. Read arguments.yaml, take the basename of its 'plotcfg' entry,
         and find that file inside the summary directory.
         Example:
           plotcfg: configs/CIT/config_dm_summary_MIB-correction.yaml
         -> config_dm_summary_MIB-correction.yaml

      2. Fallback to config_dm_summary*.yaml.

    This matters for DM because different correction variants can coexist
    (for example MIB-correction), and we want the same ranges / labels /
    fit definitions that produced the original plots.
    """
    base = Path(directory).expanduser().resolve()

    # Look for arguments.yaml first; summarize_modules.py saves it in outdir.
    arg_matches = sorted(
        base.rglob("arguments.yaml"),
        key=lambda p: (len(p.relative_to(base).parts), str(p)),
    )

    for argfile in arg_matches:
        try:
            with open(argfile, "r") as f:
                args_cfg = yaml.load(f.read()) or {}

            saved_plotcfg = args_cfg.get("plotcfg", None)

            if saved_plotcfg:
                basename = Path(str(saved_plotcfg)).name

                exact = argfile.parent / basename
                if exact.is_file():
                    return exact
            ...
            logging.warning(
                "Could not use %s to locate DM plot config: %s",
                argfile,
                exc,
            )

    # Fallback: any DM summary plot config.
    matches = sorted(
        base.rglob("config_dm_summary*.yaml"),
        key=lambda p: (len(p.relative_to(base).parts), str(p)),
    )
    ...
    if len(matches) > 1:
        logging.warning(
            "Multiple config_dm_summary*.yaml files found under %s; "
            "using %s. Pass --plotcfg explicitly if this is not the "
            "configuration used to produce the ROOT files.",
            base,
            matches[0],
        )
...
def apply_original_entry_style(hist, entrycfg):
    """Reapply the style originally specified in the DM summary plot config."""
...
def graph_ranges(plotcfg, graphs):
    """
    Reproduce summarize_modules.py's graph-range prescription, with one
    intentional BAC-comparison fix:

    For any graph whose x axis is a barcode, ALWAYS determine the x range
    from the points of *all selected BACs*, even if the single-BAC config or
    saved ROOT carries a BAC-specific xmin/xmax.

    This is needed because CIT/MIB/PKU/UVA occupy different DM-barcode
    intervals. Reusing one BAC's range would clip the other BACs.

    Y ranges still follow the original config exactly.
    """
    cfg_xmin = plotcfg.get("xmin", None)
    cfg_xmax = plotcfg.get("xmax", None)
    cfg_ymin = plotcfg.get("ymin", None)
    cfg_ymax = plotcfg.get("ymax", None)

    xtitle = str(plotcfg.get("xtitle", ""))
    is_barcode_x = "barcode" in xtitle.lower()
    ...
    # X range
    # ------------------------------------------------------------
    # Barcode plots are special for BAC comparison: ignore any BAC-specific
    # configured range and span every selected BAC.
    if is_barcode_x:
        xmin = min(all_x)
        xmax = max(all_x)

        # Keep the same outward-to-100 convention used by
        # summarize_modules.py for automatically determined large ranges.
        if abs(xmin) > 100:
        ...
        logging.info(
            "Barcode x-range from ALL BAC points: %s -> %s; individual=%s",
            xmin,
            xmax,
            per_graph_xranges,
        )
    ...
    # ------------------------------------------------------------
    # Y range: preserve the original repo/config behaviour.
    # ------------------------------------------------------------
    ymin = cfg_ymin
    ymax = cfg_ymax
    ...
    # Protect ROOT against zero-width axes.
    if xmax <= xmin:
...
def make_stacked_graph(
    ...
    For DM graphs, each original entry keeps its original meaning
    (for example Average, Top left, Top right, Bottom left, Bottom right),
    but its point set is the concatenation of the selected BACs.
    """
    out_graphs = []
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Combine existing btl-utils DM summary ROOT files (TH1 + TGraph), "
            "using the original config_dm_summary*.yaml for ranges and style."
        )
    )
    ...
    for bac in BACS:
        parser.add_argument(
            f"--{bac}",
            required=False,
            help=f"{bac} DM summary directory",
        )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional config_dm_summary*.yaml. If omitted, use the copy saved "
            "inside the first BAC DM summary directory."
        ),
    )
    ...
    if args.plotcfg:
        ...
        if plotcfg_path is None:
            parser.error(
                "Could not find config_dm_summary*.yaml in the first BAC "
    ...
    empty_root_bacs = [
        ...
        if (
            len(hist_maps[bac]) == 0
            and len(graph_maps[bac]) == 0
        )
    ]

    if empty_root_bacs:
        parser.error(
            "No h1_*.root or g1_*.root summary files were found for: "
            + ", ".join(empty_root_bacs)
            + ". This combiner needs the ROOT summary outputs, not only "
              "PDF/PNG images. Current btl-utils/utils.root_plot1D writes a "
              ".root file next to each PDF; if your copied/tarred summary "
              "folder omitted ROOT files, copy those ROOT files as well."
        )

    report = [
        "BAC DM summary merge report",
        "========================",
